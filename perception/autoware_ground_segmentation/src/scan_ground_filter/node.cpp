...
: autoware::pointcloud_preprocessor::Filter("ScanGroundFilter", options)
{
  // set initial parameters
  {
    ...
    recheck_ignore_distance_ =
      static_cast<float>(declare_parameter<double>("recheck_ignore_distance"));
    ...
    // initialize grid filter
    {
      ...
      param.recheck_ignore_distance = recheck_ignore_distance_;
  ...
  const std::vector<rclcpp::Parameter> & param)
{
  ...
  if (get_param(param, "recheck_ignore_distance", recheck_ignore_distance_)) {
    RCLCPP_DEBUG(get_logger(), "Setting recheck_ignore_distance to: %f.", recheck_ignore_distance_);
  }
