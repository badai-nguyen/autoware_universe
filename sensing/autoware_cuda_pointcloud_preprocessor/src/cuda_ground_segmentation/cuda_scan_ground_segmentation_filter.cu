...
__device__ void recheckCell(
  ...
  const size_t idx_start_point_of_cell, const FilterParameters * filter_parameters_dev,
  const int cell_idx)
{
...
__device__ void SegmentInitializedCell(
  ...
  const size_t idx_start_point_of_cell, const FilterParameters * filter_parameters_dev,
  const int sector_start_cell_index, const int cell_idx_in_sector)
{
  ...
  if (
    ...
    recheckCell(
      current_cell, classify_points, idx_start_point_of_cell, filter_parameters_dev, cell_id);
  }
}
/*
  ...
  for (int cell_index_in_sector = 0;
    ...
    if (mode == SegmentationMode::UNINITIALIZED) {
      SegmentInitializedCell(
        ...
        filter_parameters_dev, sector_start_cell_index, cell_index_in_sector);
